...
#include <utility>
#include <vector>
#include "HansDietrich\ensure.h"

#pragma warning(disable : 4996) //#define _SILENCE_CXX20_REL_OPS_DEPRECATION_WARNING
...
struct RegValue {
	...
	void Expand (void)
	{
		if (Type == REG_EXPAND_SZ)
		{
			TCHAR* b = 0;
			DWORD i = 0, ret = 0;
			DWORD size = ExpandEnvironmentStrings ((LPCTSTR)&Data[0], b, i);

			if (size > 0)
			{
				b = new TCHAR[size];

				ret = ExpandEnvironmentStrings ((LPCTSTR)&Data[0], (LPTSTR)b, size);

				if (ret == size)
				{
					Data.resize (size);
					Data.assign((BYTE*)b, (BYTE*)b + sizeof(TCHAR)*(_tcsclen(b) + 1));
				}
...
public:
	...
	LONG Create(const TCHAR* subKey, REGSAM samDesired = KEY_ALL_ACCESS)
	{
		...
		ret = RegCreateKeyEx(Hive(), subKey, 0, empty, 0, samDesired, 0, &h_, 0);
		sam_ = samDesired;
		p_ = subKey;
	...
	LONG Connect(HKEY hKey = 0, const TCHAR* machineName = 0)
	{
		LONG ret(ERROR_SUCCESS);
	...
	{
		LONG ret = RegSetValueEx(h_, valueName, 0, Value.Type, &Value.Data[0], Value.Size());
