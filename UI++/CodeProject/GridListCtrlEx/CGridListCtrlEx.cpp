...
//------------------------------------------------------------------------
bool CGridListCtrlEx::CheckOSVersion(DWORD requestOSMajor, DWORD requestOSMinor)
{
	...
	osvi.dwMajorVersion = requestOSMajor;
	osvi.dwMinorVersion = requestOSMinor;

	VER_SET_CONDITION(conditionMask, VER_MAJORVERSION, op);
	VER_SET_CONDITION(conditionMask, VER_MINORVERSION, op);

	return (VerifyVersionInfo(&osvi, VER_MAJORVERSION | VER_MINORVERSION, conditionMask) == TRUE);
...
void CGridListCtrlEx::OnCreateStyle()
{
	...
	if (CheckOSVersion(6,2) && UsingVisualStyle())
		SetExtendedStyle(GetExtendedStyle() & ~LVS_EX_GRIDLINES);
