...
CBaseEdit::CBaseEdit (std::tr1::wregex *pRegEx,
	const bool bProcessChars/* = true*/) :
	m_bProcessChars (bProcessChars),
...
void CBaseEdit::SetRegEx (std::tr1::wregex *pRegEx)
{
	if (m_bRegExOwner)
	{
		delete m_pRegEx;
...
{
	bool bSuccess = true;
	std::tr1::match_results<const TCHAR *> what;

	// This is a full match check, not a partial match
	bSuccess = std::tr1::regex_match (static_cast<const TCHAR *>(strText),
		what, *m_pRegEx, std::tr1::regex_constants::match_default);
	return bSuccess;
}
	...
	const bool bDisplayToolTip/* = true*/)
{
	...
	if (strText.GetLength ())
	{
		...
		bSuccess = std::tr1::regex_match (static_cast<const TCHAR *>(strText),
			what, *m_pRegEx, std::tr1::regex_constants::match_default /*| std::tr1::regex_constants::match_partial*/);
